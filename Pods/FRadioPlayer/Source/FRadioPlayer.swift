...
@objc public protocol FRadioPlayerDelegate: class {
    ...
    @objc func playbackProgressDidChange(_ player : FRadioPlayer)
}
...
open class FRadioPlayer: NSObject {
    ...
    public func seek(toSecond second: Int, shouldPlay: Bool = false) {
        guard let player = player, let _ = playerItem else {return}
        ...
        delegate?.playbackProgressDidChange(self)
    }
    ...
    func timerAction() {
        ...
        //        currentItem?.update()
        guard playerItem?.currentTime != nil else {return}
        delegate?.playbackProgressDidChange(self)
    }
    deinit {
        resetPlayer()
    ...
    fileprivate func startProgressTimer(){
        guard let player = player , playerItem?.duration.isValid == true else {return}
